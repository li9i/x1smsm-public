...
SMSM::SMSM()
{
  printf("[SMSM] Starting with default params \n");

  // init params
  initParams(1, 1, 0, 778, 0.20, M_PI/4, 0.0, 0.0, 0.0, 0.0, 360, 180, "SKG",
    "aces");
...
{
  printf("[SMSM] Starting with custom params \n");
...
SMSM::~SMSM()
{
  printf("[SMSM] Destroying SMSM\n");
}


/*******************************************************************************
*/
void SMSM::cacheFFTW3Plans(const unsigned int& sz)
{
  // Create forward  plan
  double* r2r_in;
  double* r2r_out;
...
*/
  std::vector<double>
SMSM::collectErrorBins(const std::vector<double>& errors)
{
  double b0 = 0.001;
  double b1 = 0.005;
  double b2 = 0.01;
...
/*******************************************************************************
*/
double SMSM::computePositionError(
...
/*******************************************************************************
*/
double SMSM::computeOrientationError(
  const std::tuple<double,double,double>& pose1,
  const std::tuple<double,double,double>& pose2)
{
  double dt = std::get<2>(pose1) - std::get<2>(pose2);
  x1smsm::Utils::wrapAngle(&dt);
...
*/
  void
SMSM::corruptRanges(std::vector<double>* scan, const double& sigma)
{
  if (sigma > 0.0)
  {
...
*/
  void
SMSM::corruptMap(std::vector< std::pair<double,double> >* map,
  const double& sigma)
{
  if (sigma > 0.0)
  {
  ...
  std::tuple<double,double,double>* virtual_pose)
{
  ...
  if (do_artificial_360)
  {
    ...
    if (completion_fashion != 5)
    {
      *real_scan = dataset_scan;
      x1smsm::ScanCompletion::completeScan(real_scan, completion_fashion);
      x1smsm::Utils::scan2points(*real_scan, dataset_pose, map);
    ...
    else
    {
      ...
      x1smsm::ScanCompletion::completeScan5(dataset_pose, dataset_scan,
        pow(2,0)*dataset_scan.size(), real_scan, map, &map_origin);
      *real_pose = map_origin;
      origin_of_generated_pose = map_origin;
    }

    *map = x1smsm::X::find(origin_of_generated_pose, *map, SIZE_MAP);
  }
  else // just impress the rays onto the plane over 2π
  {
    ...
    x1smsm::Utils::scan2points(map_scan, dataset_pose, map);

    // This is the map
    *map = x1smsm::X::find(dataset_pose, *map, SIZE_MAP);
  }
  ...
  {
    while(!x1smsm::Utils::generatePoseWithinMap(*map, radius_r, max_tries, real_pose))
    {
  ...
  while(!x1smsm::Utils::generatePose(*real_pose, *map,
  ...
  std::vector< std::pair<double,double> > intersections =
    x1smsm::X::find(*real_pose, *map, SIZE_REAL_SCAN);
  x1smsm::Utils::points2scan(intersections, *real_pose, real_scan);

#if defined (STORE)
  std::vector< std::pair<double,double> > v_intersections =
    x1smsm::X::find(*virtual_pose, *map, real_scan->size());
  std::vector<double> virtual_scan;
  x1smsm::Utils::points2scan(v_intersections, *virtual_pose, &virtual_scan);

  x1smsm::Dump::map(*map, base_path_ + "/../matlab/map_dump.m");
  x1smsm::Dump::scan(*real_scan, *real_pose, virtual_scan, *virtual_pose,
...
*/
  void
SMSM::initParams(const unsigned int& max_iterations,
  const unsigned int& num_iterations,
  const unsigned int& start_sample,
  const unsigned int& end_sample,
  const double& xy_uniform_displacement,
...
*/
  void
SMSM::invalidateRaysRandomly(std::vector<double>* scan,
...
*/
  void
SMSM::invalidateRaysSequentially(std::vector<double>* scan,
  const double& percent_invalid)
{
  if (percent_invalid > 0.0)
  {
...
void SMSM::performTests()
{
  ...
  for (int n = 0; n < NUM_ITERATIONS; n++)
  {
    ...
    for (int i = START_SAMPLE; i < END_SAMPLE; i++)
    {
      ...
      x1smsm::output_params output_params_;
      ...
      x1smsm::DatasetUtils::dataset2rangesAndPose(dataset_filepath_char,
      ...
      r = x1smsm::X::findExact(real_pose, true_map, real_scan.size());
      std::vector< std::pair<double,double> > v;
      v = x1smsm::X::findExact(virtual_pose, true_map, real_scan.size());
      x1smsm::Dump::points(r,v,0, base_path_ + "/../matlab/points_dump.m");
      x1smsm::Dump::map(true_map, base_path_ + "/../matlab/map_dump.m");
#endif
...
SMSM::testOne(
  ...
  const x1smsm::input_params& ip, x1smsm::output_params* op,
  std::tuple<double,double,double>* result_pose)
{
  // ---------------------------------------------------------------------------
  // real scan stuff
  ...
  if (METHOD.compare("SKG") == 0)
    x1smsm::Match::skg(real_scan_corrupted, real_pose, virtual_pose, map_corrupted,
